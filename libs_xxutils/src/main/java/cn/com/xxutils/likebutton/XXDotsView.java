...
public class XXDotsView extends View {
    ...
    private void updateInnerDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius2 = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0, 0.3f, 0.f, maxInnerDotsRadius);
        } else {
        ...
        if (currentProgress < 0.2) {
            ...
            this.currentDotSize2 = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0.2f, 0.5f, maxDotSize, 0.3 * maxDotSize);
        } else {
            this.currentDotSize2 = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, maxDotSize * 0.3f, 0);
        }
    ...
    private void updateOuterDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius1 = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0.0f, 0.3f, 0, maxOuterDotsRadius * 0.8f);
        } else {
            this.currentRadius1 = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0.3f, 1f, 0.8f * maxOuterDotsRadius, maxOuterDotsRadius);
        }

        if (currentProgress < 0.7) {
            this.currentDotSize1 = maxDotSize;
        } else {
            this.currentDotSize1 = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0.7f, 1f, maxDotSize, 0);
        }
    }

    private void updateDotsPaints() {
        if (currentProgress < 0.5f) {
            float progress = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0f, 0.5f, 0, 1f);
            ...
            float progress = (float) XXUtils.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, 0, 1f);
    ...
    private void updateDotsAlpha() {
        float progress = (float) XXUtils.clamp(currentProgress, 0.6f, 1f);
        int alpha = (int) XXUtils.mapValueFromRangeToRange(progress, 0.6f, 1f, 255, 0);
